          ...
          <table className="min-w-full bg-white">
            <thead>
              <tr className="w-full h-16 border-gray-300 border-b py-8">
                ...
                <th className="px-5 py-3 bg-gray-800 text-left text-xs font-semibold text-gray-300 uppercase tracking-wider rounded-tr-md">
                  Sisa Stok
                </th>
              </tr>
            </thead>
            <tbody>
              {filteredStockMovements.map((movement, index) => (
                <tr key={index} className="h-14 border-gray-300 border-b">
                  <td
                    className="px-6 cursor-pointer text-blue-600 hover:text-blue-800"
                    onClick={() => handleOrderNumberClick(movement.productId)}
                  >
                    {movement.productName}
                  </td>
